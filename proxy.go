...
// denyHTTP writes the refusal a developer will actually read. 403 with a plain-text reason: a build
// log showing "egress denied: <why>" is the difference between a policy that gets fixed and one that
// gets switched off.
func denyHTTP(w http.ResponseWriter, d Decision) {
	w.Header().Set("Content-Type", "text/plain; charset=utf-8")
	w.WriteHeader(http.StatusForbidden)
	fmt.Fprintf(w, "egress denied: %s\n", d.Reason)
}

func (p *proxy) handleConnect(w http.ResponseWriter, r *http.Request) {
	host := hostOnly(r.Host)
	if d := p.policy.CheckTunnel(host); !d.Allow {
...
func (p *proxy) serveTunnel(conn net.Conn, authority string) {
	...
	for {
		...
		if !d.Allow {
			...
			body := fmt.Sprintf("egress denied: %s\n", d.Reason)
			res := &http.Response{
				...
				// Close after a refusal: a client that keeps the connection alive and pipelines
				// more requests onto it after a 403 is not a case worth being clever about.
				Close: true,
			}
			_ = res.Write(conn)
			return
